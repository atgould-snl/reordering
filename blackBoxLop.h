...
using ViewType = Kokkos::View<double**>; //GaussSeidelBlockReorderPreconditionerFactory::BlockNormsViewType;

auto timer = easy_timer();
...
std::string to_string(TerminationReason reason)
{
  ...
  case TerminationReason::NONE:
    return "NONE";
  case TerminationReason::OPTIMAL_SOLUTION:
    return "finding optimal solution";
  case TerminationReason::MAX_WALL_TIME:
    return "hitting max walltime";
  default:
    throw "Unknown termination reason.";
    return "";
  }
}
...
{
public:
  LinearOrderingSolver(const ViewType & m_tournament,
      double m_tMaxWalltime)
...
private:
  ...
  bool update_diff_lambda(int localN, double L, double LocalbestOrder_value, bool PrepareGain)
  {
    ...
    auto handleCircuit = [&](int i, int j, int k, int Si, double coi_j, double coj_k, double coi_k)
    {
      transitive = false;
  ...
  {
    const auto tElapsed = timer.time();
...
void runBlackBoxLop(ViewType& blockNorms){
  int n=blockNorms.extent(0);
  ViewType tournament("normalForm", n, n);
  ...
  for (int a=0; a < int(n); a++){
    for (int b=0; b < int(n); b++){
      ...
      double T=0;
      T += std::pow(blockNorms(a,b),2);

      // Take away sum of block below diag
      T -= std::pow(blockNorms(b, a),2);
      tournament(a,b) = T;
      tournament(b,a) = -T;
    }
  }
  // Call the LOP solver
  LinearOrderingSolver solver{tournament, 100000}; // Is communicator pulled in from the namespace??
  solver.solve();
};
